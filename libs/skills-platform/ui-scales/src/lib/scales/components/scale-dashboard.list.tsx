...
import { Scale } from '@skills/data-access';
import clsx from 'clsx';

export interface ScaleDashboardListProps {
  allScales: Scale[];
  scalesRef: MutableRefObject<(HTMLAnchorElement | null)[]>;
  selectedScale?: Scale;
  onSelectScale: (scale: Scale) => void;
}

export const ScaleDashboardList = forwardRef(
  (
    { allScales: registry, scalesRef, selectedScale, onSelectScale }: ScaleDashboardListProps,
    ...
    <ul className="tw-space-y-14 tw-divide-y tw-divide-neutral-200 lg:tw-space-y-12" ref={forwardedRef}>
      {registry.map((scale, index) => (
        <li key={scale.id} className="tw-pt-14 first-of-type:tw-pt-0 lg:tw-pt-12">
          <Link
            ref={(el) => (scalesRef.current[index] = el)}
            to={scale.id}
            className={clsx(
              '-tw-mx-4  tw-flex tw-flex-col tw-gap-4 tw-border-y tw-px-4 tw-py-6 hover:tw-border-neutral-300 hover:tw-bg-white focus:tw-border-neutral-300 focus:tw-bg-white focus:tw-outline-none active:tw-border-neutral-600 active:tw-bg-white md:-tw-mx-6 md:tw-rounded-xl md:tw-border-x md:tw-px-6',
              scale.id === selectedScale?.id ? 'tw-border-neutral-600 tw-bg-white' : 'tw-border-transparent'
            )}
            onClick={() => onSelectScale(scale)}
          >
            <h3 className="tw-flex tw-items-center tw-gap-x-3 tw-text-2xl tw-font-bold tw-text-neutral-900">
              {/* Degreed logo to be used with default Degreed scales. TODO: Get this as a CDN asset filename from the scale. */}
              {scale.name === 'Degreed' ? (
                <svg className="tw-h-6 tw-w-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
            ...
            <div className="tw-flex tw-items-center">
              ...
              <div className="tw-text-xs tw-font-extrabold tw-text-neutral-500">{scale.totalLevelCount} Levels</div>
            </div>
            <p className="tw-text-neutral-700">{scale.description}</p>
          </Link>
        </li>
      ))}
    </ul>
