import { initElfStoreState } from './store.model';
...
import { createStore, emitOnce, Store, withProps } from '@ngneat/elf';
import {
  deleteAllEntities,
  getActiveIds,
  resetActiveIds,
...
import { StoreState, updateRequestStatus } from '../_shared';
...
export interface PagedEntityState<K extends Entity> {
...
export class PaginatedStore<T extends StoreState, K extends Entity> extends ReactiveStore<T, K> {
   ...
   */
  constructor(storeName: string, initState: () => Partial<T>) {
    ...
    this._store = createStore(
      ...
      withProps<T>({ ...initState(), ...initElfStoreState() }), // Store State
      ...
      map(({ _, ...data }) => {
        ...
        return {
          ...
          ...{
            ...
            numPages: pagination.perPage ? Math.ceil((pagination.total || 0) / pagination.perPage) : 0,
            showPage,
          },
        };
      }),
  ...
  /**
   * Pages are stored in the pagination area as an array of IDs
   * Use entity IDs to retrieve the actual entity data.
   */
  public getPagedEntities(page?: number): K[] {
    const state = this.getState() as T & PagedEntityState<K>;
    const pages = state.pagination.pages || {};
    const pagedIds = pages[page || state.pagination.currentPage] || [];

    return pagedIds.map((id) => state.entities[id]);
  }
  ...
  public override reset() {
    const initState = (s: T) => ({ ...s, ...this.initState(), ...initElfStoreState() });
  ...
  /**
   * Add or remove item from 'active' selection
   */
  public markAsSelected(partialId: string, selected = true, excludeOthers = true): void {
    const skill = this.findItemByID(partialId);
    if (skill) {
      const id = skill.id;

      const calculateActiveIds = () => {
        const actives = excludeOthers ? [] : this._store.query(getActiveIds);
        const isSelected = actives.indexOf(id) > -1;

        if (selected && !isSelected) return [...actives, id];
        if (!selected && isSelected) return actives.filter((it) => it !== id);

        return actives;
      };

      this._store.update(resetActiveIds(), setActiveIds(calculateActiveIds()));
    }
  }
