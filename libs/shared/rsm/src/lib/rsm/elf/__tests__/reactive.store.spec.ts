import { getRequestStatus, StatusState, StoreState } from '../../_shared';
import { ReactiveStore } from '../reactive.store';
import { initElfStoreState } from '../store.model';
import { DATA, ListItem } from './_data_/list-item.data';
...
class MyStore extends ReactiveStore<MyStoreState, ListItem> {
  constructor() {
    super(STORE_NAME, () => ({ ...initElfStoreState(), user: '' }));
  }
}
...
describe('ReactiveStore', () => {
  ...
  describe('initialization', () => {
    ...
    it('should have API', () => {
      ...
      expect(store).toHaveObservables(['state$']);
    });
  ...
  describe('status', () => {
    ...
    it('should initialize as "idle"', () => {
      expect(status()).toBe('initializing'); // default for stores with entity management
