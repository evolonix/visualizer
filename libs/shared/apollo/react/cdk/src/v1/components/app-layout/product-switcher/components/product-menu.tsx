import { Disclosure, DisclosurePanel, PopoverButton } from '@headlessui/react';
...
export function ProductMenu({ item, open, collapsible, onClick }: ProductMenuProps) {
        ...
        <div>
          ...
          >
            ...
            {collapsible ? (
              ...
              <ChevronRightIcon className={clsx(open ? 'rotate-90' : '', 'pointer-events-none size-4')} aria-hidden="true" />
            ) : null}
          </button>
        </div>

        <DisclosurePanel id={`${item.product}-disclosure-panel`} as="ul" static className="border-t border-neutral-300" hidden={!open}>
          {item.subItems.map((subItem) => (
            <li key={subItem.text}>
              ...
              <PopoverButton
                ...
                className={clsx(
                  subItem === activeChild ? 'font-extrabold text-neutral-900' : 'font-semibold text-neutral-600',
                  'flex h-14 w-full items-center gap-6 bg-neutral-100 px-6 py-3 text-left hover:bg-neutral-200 hover:text-neutral-800 focus:bg-neutral-200 focus:text-neutral-800 focus:outline-none active:bg-neutral-200 active:text-neutral-900'
                )}
              >
          ...
          <ul className="border-t border-neutral-300">
            <li>
              <PopoverButton
                ...
                className="flex h-14 w-full items-center gap-6 bg-neutral-100 px-6 py-3 text-left font-extrabold text-neutral-900 hover:bg-neutral-200 hover:text-neutral-800 focus:bg-neutral-200 focus:text-neutral-800 focus:outline-none active:bg-neutral-200 active:text-neutral-900"
              >
                <div className="grid size-8 flex-shrink-0 place-items-center">
