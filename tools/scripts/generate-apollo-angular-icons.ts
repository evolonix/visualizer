 ...
 *
 *  Apollo uses HerioIcons (svgs) for its icons.
 *
 *  This utility will generate
 *   - the Angular components for each icon [in the HeroIcons library],
 *   - the IconComponent, and
 *   - the IconsModule.
 *  Note: Each Angular component will contain the solid + outline, 16/20/24 sizes of the icon
 *
 *  The `<da-icon>` component is the primary component that will internally support all Apollo icons.
 */
...
async function generateBaseIconComponent(targetDir: string) {
  ...
  `;
  const baseIconPath = join(targetDir, '_base.component.ts');
  await writeFile(baseIconPath, baseIconContent, 'utf8');

  console.log('Generated base icon component');
}

async function generateIconComponent(targetDir: string) {
  // Generate icon component
  const iconContent = outdent`
  ...
  `;
  const iconPath = join(targetDir, 'icon.component.ts');
  await writeFile(iconPath, iconContent, 'utf8');

  console.log('Generated icon component');
}

async function generateIconComponents(sourceDir: string, targetDir: string) {
  const svgContents: { [key: string]: SvgProp[] } = {};

  // Loop through the optimized directory to get the size (16, 20, 24) directories
  const sizes = await readdir(sourceDir, { withFileTypes: true });
  for (const size of sizes) {
    ...
    const types = await readdir(join(sourceDir, size.name), {
      withFileTypes: true,
    });
    for (const type of types) {
      ...
      const files = await readdir(join(sourceDir, size.name, type.name));
      for (const file of files) {
        let svgContent = await readFile(join(sourceDir, size.name, type.name, file), 'utf8');
        svgContent = svgContent.replace('data-slot="icon"', 'data-slot="icon" [class]="className"');
  ...
  for (const [componentName, props] of Object.entries(svgContents)) {
    ...
    const componentContent = outdent`
      ...
      import { BaseIconComponent } from './_base.component';
    ...
    const componentPath = join(targetDir, `${componentName}.ts`);
...
async function generateIconsModule(targetDir: string, svgContents: { [key: string]: SvgProp[] }) {
  ...
  const moduleContent = outdent`
    ...
    ${Object.keys(svgContents)
      .map((componentName) => `import { ${toPascalComponentName(componentName)}IconComponent } from './components/${componentName}';`)
      .join('\n')}
    ...
    @NgModule({
      declarations: [
        IconComponent,
        ${Object.keys(svgContents)
          .map((componentName) => `${toPascalComponentName(componentName)}IconComponent,`)
          .join('\n\t\t')}
      ],
...
async function generateIconComponentsRegistry(targetDir: string, svgContents: { [key: string]: SvgProp[] }) {
  // Generate icon components registry
  const iconComponentsRegistryContent = outdent`
    import { Type } from '@angular/core';

    ${Object.keys(svgContents)
      .map((componentName) => `import { ${toPascalComponentName(componentName)}IconComponent } from './${componentName}';`)
      .join('\n')}

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const iconComponentsRegistry: { [key: string]: Type<any> } = {
      ${Object.keys(svgContents)
        .map((componentName) => `'${componentName}': ${toPascalComponentName(componentName)}IconComponent,`)
        .join('\n\t')}
    };
  ...
  await writeFile(iconComponentsRegistryPath, iconComponentsRegistryContent, 'utf8');

  console.log('Generated icon components registry');
}

async function generateIconsBarrelFile(targetDir: string) {
...
async function main() {
  // Get the path to the heroicons and target icons directories from arguments
  ...
  if (!heroiconsDir || !iconsDir) {
    console.log(outdent`
      ...
      npx tsx tools/scripts/generate-apollo-angular-icons.ts tmp/heroicons libs/shared/apollo/angular/src/lib/icons
  ...
  await generateIconComponent(iconsDir);

  const svgContents = await generateIconComponents(optimizedDir, componentsDir);
  await generateIconsModule(iconsDir, svgContents);
  await generateIconComponentsRegistry(componentsDir, svgContents);
  await generateIconsBarrelFile(iconsDir);

  // Format the generated files
  console.log('Formatting the generated files with Prettier');
